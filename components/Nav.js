...
const Nav = () => {
  ...
  return (
    <nav className="flex flex-col items-center xl:justify-center gap-y-4 fixed h-max bottom-0 mt-auto xl:right-[2%] z-50 top-0 w-full xl:w-16 xl:max-w-md xl:h-screen">
      ...
      xl:justify-center gap-y-10 px-4 md:px-40 xl:px-0 h-[50px] xl:h-max py-8 bg-white/10
        ...
        {navData.map((link, index) => {
          return <Link className={`${link.path === pathname && "text-accent"} relative flex items-center group hover:text-accent transition-all duration-300`} key={index} href={link.path}>
            ...
            <div className="absolute pr-14 right-0 hidden xl:group-hover:flex">
              ...
              {/* triangle */}
              <div className="border-solid border-l-white border-l-8 border-y-transparent border-y-[6px] border-r-0 absolute right-12 top-1.5"></div>
            </div>
